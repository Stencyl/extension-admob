...
- (void)loadWithFormInformation:(nonnull NSDictionary<PACFormKey, id> *)formInformation
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    self->_formInformation = [formInformation mutableCopy]; // FIX FOR WEB VIEW LOADING
    //self->_loadCompletionHandler = wrappedHandler;
    self->_loadCompletionHandler = [wrappedHandler copy]; // FIX FOR LOADING CALLBACK
    [self loadWebView];
  });
}
...
/// Loads the consent form HTML into the web view. (default version)
/*- (void)loadWebView {
...
/// Loads the consent form HTML into the web view. (modified version)
- (void)loadWebView {
  NSFileManager *filemgr = [[NSFileManager alloc] init];
  NSString *filemgrPath = [filemgr currentDirectoryPath];
  NSURL *baseURL = [NSURL fileURLWithPath:filemgrPath];
  NSURL *URL = [NSURL URLWithString:@"assets/consentform.html" relativeToURL:baseURL];
  NSURLRequest *URLRequest = [[NSURLRequest alloc] initWithURL:URL];
  [_webView loadRequest:URLRequest];
}
